...
const collaspseContext = inject(collapseContextKey)
const isActive = computed(() => collaspseContext?.activeNames.value.includes(props.name))

const handleClick = () => {
  if (props.disabled) {
    return
  }
  collaspseContext?.handleItemClick(props.name)
}
